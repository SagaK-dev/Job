...
final class JobCommand implements CommandExecutor, TabCompleter {
    private static final List<String> PLAYER_ROOTS = List.of(
            "shop", "balance", "money", "rates", "status", "help", "version"
    );
    private static final List<String> ADMIN_ROOTS = List.of("rate", "setrate", "reload");
    ...
    JobCommand(JobPlugin plugin, BeginnerService beginners, RewardService rewards,
               RateTable rateTable, YenService yen, JobSellMenu sellMenu) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        return switch (action) {
            case "balance", "money" -> handleBalance(sender);
            case "rates" -> {
                if (!require(sender, "job.use")) yield true;
                sendRates(sender);
                yield true;
            }
            case "status" -> handleStatus(sender);
            case "rate", "setrate" -> handleRate(sender, label, args);
            case "reload" -> {
                if (!require(sender, "job.admin")) yield true;
            ...
            case "version" -> {
                sender.sendMessage("§6Job §f" + plugin.getDescription().getVersion()
                        + " §7(target: Spigot 1.21.1 / Java 21)");
                sender.sendMessage("§7独自通貨: §e円 §7| GUI: §f/" + label);
                yield true;
            }
            default -> {
    ...
    private boolean openShop(CommandSender sender) {
        ...
        if (!(sender instanceof Player player)) {
            sender.sendMessage("§c売却GUIはプレイヤーのみ使用できます。 /job rates はコンソールでも使用できます。");
            return true;
        }
    ...
    private boolean handleBalance(CommandSender sender) {
        ...
        player.sendMessage("§6[Job] 所持金: §e" + YenService.format(yen.balance(player)));
        return true;
    }
    ...
    private boolean handleRate(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 4) {
            ...
            if (parsed == null || !RateTable.isValidYen(parsed)) {
            ...
            if (parsedAmount == null || !RateTable.isValidYen(parsedAmount)) {
                sender.sendMessage("§c金額は1〜" + RateTable.MAX_YEN_PER_UNIT + "の整数で指定してください。");
                return true;
            }
        ...
        sender.sendMessage("§a[Job] " + rule.configKey() + " を §f" + unitCount + "個=§e"
                + amount + "円§a に変更しました。GUI売却価格にも即時反映されます。");
        return true;
    }
    ...
    private void sendRates(CommandSender sender) {
        ...
        sender.sendMessage("§6初心者（累計プレイ時間が" + beginners.playtimeHours()
                + "時間未満）は支払額×" + beginners.beginnerMultiplier() + "。");
    }
    ...
    private void sendStatus(Player player) {
        ...
        if (beginners.isBeginner(player)) {
            ...
            player.sendMessage("§6初心者ボーナス: ×" + beginners.beginnerMultiplier()
                    + " §f(累計 " + (playedMinutesTotal / 60L) + "時間" + (playedMinutesTotal % 60L)
                    + "分 / " + beginners.playtimeHours() + "時間、残り "
                    + (remainingMinutes / 60L) + "時間" + (remainingMinutes % 60L) + "分)");
        } else {
    ...
    private static void sendUnknownRule(CommandSender sender) {
        sender.sendMessage("§c不明な単価キーです。使用可能: "
                + String.join(", ", Arrays.stream(RewardRule.values()).map(RewardRule::configKey).toList()));
    }
    ...
    private static void sendUsage(CommandSender sender, String label) {
        sender.sendMessage("§6Job commands");
        sender.sendMessage("§e/" + label + " §7- 売却GUIを開く");
        sender.sendMessage("§e/" + label + " balance §7- 所持円を確認");
        sender.sendMessage("§e/" + label + " rates §7- 現在の単価表");
        sender.sendMessage("§e/" + label + " status §7- 所持円・初心者倍率・進捗");
        sender.sendMessage("§e/" + label + " version §7- 導入中のJobバージョンを確認");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            List<String> roots = new ArrayList<>(PLAYER_ROOTS);
            if (sender.hasPermission("job.admin")) {
                roots.addAll(ADMIN_ROOTS);
            }
        ...
        if (args[0].equalsIgnoreCase("setrate") && args.length == 2) {
