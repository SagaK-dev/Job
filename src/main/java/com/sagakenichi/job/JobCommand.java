...
final class JobCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleRate(CommandSender sender, String label, String[] args) {
        ...
        if (args[1].equalsIgnoreCase("get")) {
            ...
            if (rule == null) { sendUnknownRule(sender); return true; }
        ...
        if (args.length == 4) {
            ...
            Integer count = parsePositiveInt(args[3]);
            Integer amount = parsePositiveInt(args[4]);
            if (count == null || !RateTable.isValidUnitCount(count)) {
                sender.sendMessage("§c必要個数は1〜" + RateTable.MAX_UNIT_COUNT + "の整数で指定してください。");
                return true;
            }
            if (amount == null || !RateTable.isValidYen(amount)) {
                sender.sendMessage("§c金額は1〜" + RateTable.MAX_YEN_PER_UNIT + "の整数で指定してください。");
                return true;
            }
            unitCount = count;
    ...
    private void sendRates(CommandSender sender) {
        ...
        sender.sendMessage("§6初心者（初回ログインから" + beginners.durationHours()
                + "時間以内）は上記の支払額×" + beginners.beginnerMultiplier() + "。");
    }
    ...
    private static Integer parsePositiveInt(String raw) {
        try { int value = Integer.parseInt(raw); return value > 0 ? value : null; }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            ...
            if (sender.hasPermission("job.admin")) {
                roots.add("rate"); roots.add("setrate"); roots.add("reload");
            }
            return startsWith(roots, args[0]);
        }
        if (!sender.hasPermission("job.admin")) return List.of();
        if (args[0].equalsIgnoreCase("rate")) {
            if (args.length == 2) return startsWith(List.of("list", "get", "set"), args[1]);
        ...
        if (args[0].equalsIgnoreCase("setrate")) {
            if (args.length == 2) return startsWith(ruleKeys(), args[1]);
            if (args.length == 3) return startsWith(List.of("1", "2", "4", "12"), args[2]);
        }
        return List.of();
    }
